(() => {
  ...
  function openCodexPlusModal() {
    ...
    overlay.innerHTML = `
      <div class="codex-plus-modal-content" role="dialog" aria-modal="true" aria-label="Codex Mate">
        <div class="codex-plus-modal-header">
          <div class="codex-plus-modal-title">Codex Mate ${codexPlusVersion}</div>
        ...
        <div class="codex-plus-modal-body">
          ...
          <div class="codex-plus-row">
            <div><div class="codex-plus-row-title">原生菜单栏位置</div><div class="codex-plus-row-description">把 Codex Mate 菜单插入顶部原生菜单栏；默认关闭以避免页面重渲染冲突。</div></div>
            <button type="button" class="codex-plus-toggle" data-codex-plus-setting="nativeMenuPlacement"><span></span></button>
          </div>
          <div class="codex-plus-row">
            <div><div class="codex-plus-row-title">关于 Codex Mate</div><div class="codex-plus-about">Codex Mate 是通过外部 launcher 注入的增强菜单，不修改 Codex App 原始安装文件。<br>GitHub: <a href="https://github.com/serein431/Codex-Mate" target="_blank" rel="noreferrer">https://github.com/serein431/Codex-Mate</a></div></div>
          </div>
          <div class="codex-plus-row">
  ...
  function removeDuplicateCodexPlusMenus(keep) {
    ...
    Array.from(document.querySelectorAll("button")).forEach((button) => {
      if ((button.textContent || "").trim() === `Codex Mate ${codexPlusVersion}` && !button.closest(`#${codexPlusMenuId}`)) {
        button.remove();
      }
    });
  }
  ...
  function installCodexPlusMenu() {
    ...
    trigger.type = "button";
    trigger.textContent = `Codex Mate ${codexPlusVersion}`;
