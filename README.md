...
- 完整对话目录：读取本地 rollout，长对话也能看到完整用户提问目录
- 删除前确认，并支持撤销
...
2.1 重点修复长对话导航问题：不再只依赖当前页面已经渲染出来的几条消息，而是读取本机 rollout 文件生成完整用户提问目录。

- 新增完整对话目录：CM 面板默认开启，右侧目录会展示当前会话里的完整用户提问。
- 改进长对话定位：点击目录项会先匹配当前已渲染消息，找不到时按完整进度滚动到附近，并在页面加载后继续重试定位。
- 保留滚动位置恢复：目录跳转会短暂暂停恢复逻辑，避免刚跳到目标位置又被拉回上次阅读处。
- 保持只读：目录只读取 `state_5.sqlite` 和 rollout 文件，不会改写聊天记录。
...
Codex Mate 会读取本机 `state_5.sqlite` 和当前会话对应的 rollout 文件，生成完整用户提问目录。这个目录不依赖当前页面已经渲染出多少条消息，所以长对话也能看到完整提问列表。

点击目录项时，Codex Mate 会先尝试匹配当前已经渲染出来的用户消息；如果目标消息还没挂载到页面里，会先按完整进度滚动到附近，再等待页面加载后继续定位。整个过程只读取本机数据，不会改写聊天记录。
